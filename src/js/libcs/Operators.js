...
function comp_equals(args, modifs) {
  ...
  if (v0.ctype === v1.ctype) {
    if (v0.ctype === "number") {
      return General.bool(
        v0.value.real === v1.value.real && v0.value.imag === v1.value.imag
      );
    }
    if (v0.ctype === "string") {
  ...
  for (;;) {
    ...
    for (i1 = 0; i1 < n; ++i1) {
      for (i2 = 0; i2 < n; ++i2) {
        ...
        if (
          e[i1][i2] <
          (abs(w[i1][i2]) + abs(v1[i1].cost) + abs(v2[i2].cost)) * 1e-14
        )
          e[i1][i2] = 0;
      }
    }
...
evaluator.elementsatmouse$0 = function (args, modifs) {
  ...
  var lines = csgeo.lines.filter(function (el) {
    ...
    if (val && el.kind === "S") {
      ...
      var cr = List.crossratio3(el.farpoint, el.startpos, el.endpos, mouse, tt)
        .value.real;
    ...
    if (!match[4]) {
      ...
      if (ncols < row.length) {
        ...
        for (var i = 0; i < data.length; ++i)
          for (var j = data[i].length; j < ncols; ++j) data[i][j] = nada;
...
evaluator.removeelement$1 = function (args, modifs) {
  ...
  if (arg.ctype === "geo") removeElement(arg.value.name);
    ...
    console.log("argument of removeelement is undefined or not of type <geo>");
};
