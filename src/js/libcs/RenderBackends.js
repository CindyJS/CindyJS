...
SvgWriterContext.prototype = {
  ...
  restore: function () {
    ...
    if (this._saveStack.length === 0) this._saveStack.push("");
  },
  ...
  drawImage: function (img, x, y) {
    if (arguments.length !== 3)
      throw Error(
        "SvgWriterContext only supports " + "3-argument version of drawImage"
      );
...
PdfWriterContext.prototype = {
  ...
  _kappa: 0.5522847498307934, // 4 * (Math.sqrt(2) - 1) / 3
  ...
  _png: function (dataURL) {
    ...
    if (compressionMethod !== 0)
      throw Error("Unsupported PNG compression method: " + compressionMethod);
    if (filterMethod !== 0)
  ...
  drawImage: function (img, x, y) {
    if (arguments.length !== 3)
      throw Error(
        "PdfWriterContext only supports " + "3-argument version of drawImage"
      );
    var idx = this._imgcache.indexOf(img);
    if (idx === -1) {
      idx = this._imgcache.length;
  ...
  _obj: function (obj, idx) {
    ...
    if (!Array.isArray(obj)) obj = [this._dict(obj)];
    obj.index = idx;
    obj.ref = idx + " 0 R";
