...
const supDigit = "[⁰¹²³⁴⁵⁶⁷⁸⁹]";
const subDigit = "[₀₁₂₃₄₅₆₇₈₉]";
const supNum = expandSpaces("(?:[⁺⁻] )?" + supDigit + "(?: " + supDigit + ")*");
const subNum = expandSpaces("(?:[₊₋] )?" + subDigit + "(?: " + subDigit + ")*");
...
const unicodeLetters = (function (dict, str, hiRanges) {
    let i, fst;
    let j = 0;
    let res = "(?:[";
    const n = str.length;
    ...
    [
        2, 0, 106, 3, 4, 1, 6, 5, 7, 11, 17, 8, 12, 21, 9, 22, 30, 10, 15, 24, 25, 16, 13, 42, 46, 14, 18, 19, 29, 37,
        27, 28, 35, 26, 32, 36, 40, 43, 47, 53, 20, 48, 50, 56, 33, 34, 39, 51, 52, 55, 63, 64, 65, 68, 85, 23, 31, 38,
        45, 49, 105, 59, 66, 69, 72, 88, 102, 114, 117, 128, 157, 191, 41, 44, 54, 60, 67, 70, 71, 73, 74, 75, 76, 80,
        81, 82, 83, 84, 86, 87, 89, 93, 94, 98, 99, 107, 108, 116, 122, 130, 132, 134, 138, 160, 165, 185, 195, 196,
        255, 268, 277, 310, 332, 339, 362, 365, 390, 449, 457, 470, 512, 513, 541, 568, 582, 619, 673, 726, 768, 820,
        921, 991, 1164, 2684, 6581, 8453, 11171, 20949,
    ],
...
const reIdentifier = expandSpaces("#(?: [1-9])?|(?:'|" + unicodeLetters + ")(?: (?:[0-9']|" + unicodeLetters + "))*");

const reExponent = "(?: [Ee](?: [+-])?(?: [0-9])+)";
...
function Tokenizer(input) {
    ...
    const bols = []; // beginnings of lines
...
function applyOperator(seq) {
    const op = seq[seq.length - 2];
    const lhs = seq[seq.length - 3];
    const rhs = seq[seq.length - 1];
    if (lhs) {
...
function subsup(seq, tok, op, dict) {
    ...
    let dstDigits = "";
    const srcDigits = tok.text;
    for (let i = 0; i < srcDigits.length; ++i) dstDigits += "0123456789+-".charAt(dict.indexOf(srcDigits.charAt(i)));
    tok.ctype = "number";
    tok.value = {
        real: +dstDigits,
        imag: 0,
...
function parseRec(tokens, closing) {
    ...
    const seq = [];
    let tok; // last token to be processed
    let parsejsonatom = closing === "}";
...
Parser.prototype.postprocess = function (expr) {
    ...
    if (expr) {
        ...
        if (expr.modifs) for (const key in expr.modifs) expr.modifs[key] = this.postprocess(expr.modifs[key]);
...
Parser.prototype.parse = function (code) {
    try {
        const res = parseRec(new Tokenizer(code));
