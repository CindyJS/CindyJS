...
(function initializeOperators() {
    let precedence = 0;
    for (let level of operatorLevels) {
        precedence += 2;
        let rassoc = !!level.rassoc;
        for (let name in level) {
            let symbols = level[name];
    ...
    for (let op of prefixOnly) {
        operators[op].prefix = true;
        operators[op].infix = false;
    }
    for (let op of postfixOnly) {
        operators[op].postfix = true;
        operators[op].infix = false;
    }
    for (let op of flexfix) {
        operators[op].prefix = true;
        operators[op].postfix = true;
...
const reNextToken = [ //               0: token text
...
const reSpace = new RegExp(whitespace + '+', 'g');
...
(function sanityCheck() {
    let re = new RegExp(reNextToken, 'g');
    let match = re.exec('');
...
class Tokenizer {
    ...
    nextInternal() {
        let match = this.re.exec(this.input);
...
function applyOperator(seq) {
    let op = seq[seq.length - 2];
    let lhs = seq[seq.length - 3];
    let rhs = seq[seq.length - 1];
    if (lhs) {
        if (rhs) { // expr op expr
            if (!op.op.infix)
                throw ParseError(
        ...
        switch (tok.toktype) {
            ...
            case 'BRA':
                ...
                if (tok.text === closing || bra & 1)
                    break parseLoop;
                let closer = brackets.charAt(bra + 1);
                let sub = parseRec(tokens, closer);
                let ctok = sub.closedBy;
                if (ctok.text !== closer)
                    throw ParseError(
                        'Opening ' + tok.text +
...
function parse(code) {
    try {
        let res = parseRec(new Tokenizer(code));
