...
function applyOperator(seq) {
  ...
  if (lhs) {
    ...
    if (rhs) {
      // expr op expr
      if (!op.op.infix)
        throw ParseError("Operator may not be used infix", op.start, op.text);
    } else {
      // expr op null
      if (!op.op.postfix)
        throw ParseError("Operator may not be used postfix", op.start, op.text);
    }
  } else {
    if (rhs) {
      // null op expr
      if (!op.op.prefix)
        throw ParseError("Operator may not be used prefix", op.start, op.text);
    } else {
      if (!op.op.bare)
        throw ParseError("Operator without operands", op.start, op.text);
    }
  }
  op.ctype = "infix";
  op.oper = op.op.sym;
        ...
        if (!(seq.length & 1)) {
          ...
          if (pair === "||") {
            if (lst.length === 1) {
              ...
              throw ParseError(
                "Don't support |…| with " + lst.length + " arguments",
                tok.start
              );
            }
          } else if (pair === "{}") {
