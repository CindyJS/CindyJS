...
(function initializeOperators() {
    ...
    prefixOperators.forEach(op => {
        operators[op].prefix = true;
    });
    prefixOnly.forEach(op => {
        operators[op].prefix = true;
        operators[op].infix = false;
    });
    postfixOnly.forEach(op => {
...
operatorSymbols.sort((a, b) => b.length - a.length);
...
const unicodeLetters = ((dict, str, hiRanges) => {
    let i;
    let fst;
    let j = 0;
    let res = "(?:[";
...
const reIdentifier = expandSpaces(
...
const reNumber = expandSpaces(
    ...
    `(?:[0-9](?: [0-9])*(?: \\.(?! \\.)(?:(?! ${reIdentifier})|(?= ${reExponent}))(?: [0-9])*)?|\\.(?: [0-9])+)${reExponent}?`
);

const reNextToken = [ //                 token text
...
function subsup(seq, tok, op, dict) {
    ...
    for (let i = 0; i < srcDigits.length; ++i)
    ...
    parseLoop: while (true) {
        ...
        switch (tok.toktype) {
            ...
            case 'SUP':
                ...
                break;
            case 'BRA':
                const bra = brackets.indexOf(tok.text);
                ...
                if (ctok.text !== closer)
                    throw ParseError(
                        `Opening ${tok.text} at ${tok.start.row}:${tok.start.col} closed by ${ctok.text || 'EOF'} at ${ctok.start.row}:${ctok.start.col}`);
                ...
                if (!(seq.length & 1)) { // value position
                    if (pair === '||') {
                        if (lst.length === 1) {
                            ...
                            throw ParseError(
                                `Don't support |…| with ${lst.length} arguments`, tok.start);
                        }
                    } else if (pair === '{}') {
                        seq.push({
                            ctype: 'function',
        ...
        if (expr.ctype === 'infix') {
            if (expr.oper === ':=') {
                ...
                if (fun.ctype === 'function') {
                    ...
                    throw ParseError(
                        `${expr.oper} can only be used to define functions or variables`,
                        expr.start);
                }
            } else if (expr.oper === ',') {
                throw ParseError(
...
if (typeof process !== "undefined" &&
    ...
    module.exports.parse = code => (new Parser()).parse(code);
}
