...
function addElementNoProof(el) {
  ...
  if (op.signature !== "**") {
    ...
    if (!Array.isArray(op.signature) && op.signature.charAt(1) === "*") {
      ...
      el.args.forEach(function (val) {
        if (csgeo.csnames[val].kind !== op.signature.charAt(0)) {
          ...
          if (typeof window !== "undefined")
            window.alert(
              "Not all elements in set are of same type: " + el.name
            );
          return null;
        }
      });
      ...
      console.error(
        "Wrong number of arguments for " + el.name + " of type " + el.type
      );
      if (typeof window !== "undefined")
  ...
  if (el.args) {
    for (i = 0; i < el.args.length; ++i) {
      if (!csgeo.csnames.hasOwnProperty(el.args[i])) {
        console.log(
          "Dropping " + el.name + " due to missing argument " + el.args[i]
        );
        return null;
      }
  ...
  var nameCmp = function (cmpMap, arrn) {
    return function (setel) {
      if (cmpMap[setel.name]) {
        if (debug)
          console.log("Removed element " + setel.name + " from " + arrn);
        return false;
      } else return true;
    };
  };
