...
function createCindyNow() {
    // if(instanceInvocationArguments.use && instanceInvocationArguments.use.includes("katex")) {
    //     loadKatex();
    // } else {
    //     loadPlugins();
    // }
...
function loadKatex() {
    ...
    var defaultFonts = [
        "KaTeX_AMS:n4",
        "KaTeX_Caligraphic:n4",
        "KaTeX_Caligraphic:i4",
        "KaTeX_Fraktur:n4",
        "KaTeX_Fraktur:i4",
        "KaTeX_Main:n4",
        "KaTeX_Main:i4",
    ...
    function fontLoaderReady() {
        ...
        loadFonts(defaultFonts); // pre-load common fonts
        someScriptLoaded();
    }
    ...
    function fontActive(font, variant) {
        ...
        if (numberOfFontsLoaded === defaultFonts.length) {
            loadPlugins();
        }
    }
...
function loadPlugins() {
    ...
    if (instanceInvocationArguments.use) {
        instanceInvocationArguments.use.forEach(function (name) {
            if(name === "katex") {
                if (instanceInvocationArguments.use.length >= 1) return;
                loadCindyScript();
            }
            ...
            if (!cb) {
                ...
                CindyJS.loadScript(
                    ...
                    function () {
                        ...
                        if (--waitForPlugins === 0 && startupCalled) loadCindyScript();
                    },
                    function () {
...
async function loadCindyScript() {
    startupCalled = true;
