...
function simpl(expr) {
    ...
    switch (expr.ctype) {
        ...
        case "function":
            var modifs = {};
            for (var key in expr.modifs) modifs[key] = simpl(expr.modifs[key]);
...
describe("CindyScript tokenizer", function () {
    ...
    it("should reconstruct regular expression for unicode letters", function () {
        ...
        expect(esc(Parser.unicodeLetters)).to.deep.equal(
            esc(
                (expected =
        ...
        // Second test to ward against errors introduced by esc
        expect(Parser.unicodeLetters).to.deep.equal(expected);
    });
});
