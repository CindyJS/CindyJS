...
function Visitor(head, body) {
    this.name = head.name;
    var args = head.args.map(function (arg) {
        return arg.name;
    });
    this.local = {};
    for (var i = 0; i < args.length; ++i) this.local[args[i]] = args[i];
...
Visitor.prototype.statement = function (expr) {
    ...
    if (res !== null) jscode.push(this.indent + res + ";");
};

Visitor.prototype.visit = function (expr) {
    if (typeof expr === "string") return expr; // presumably already converted.
    switch (expr.ctype) {
        case "infix":
            switch (expr.oper) {
                case ";":
                ...
                case "=":
                    if (expr.args[0].ctype === "variable") {
                        if (this.isLocal(expr.args[0].name)) {
                            return this.local[expr.args[0].name] + " = " + this.visit(expr.args[1]);
                        } else {
                            return this.jscall("namespace.setvar", [JSON.stringify(expr.args[0].name), expr.args[1]]);
                        }
                    } else {
        ...
        case "variable":
            if (this.isLocal(expr.name)) {
                ...
                return this.jscall("namespace.getvar", [JSON.stringify(expr.name)]);
            }
        case "number":
            return this.jscall("CSNumber.complex", [expr.value.real, expr.value.imag].map(String));
        case "string":
...
Visitor.prototype.jscall = function (name, args) {
    return [name, "(", args.map(this.visit, this).join(", "), ")"].join("");
};
...
Visitor.prototype.fun_regional = function (args, modifs) {
    ...
    for (var i = 0; i < vars.length; ++i) this.local[vars[i]] = vars[i];
    return "var " + vars.join(", ");
};

Visitor.prototype.fun_genList = function (args, modifs) {
    return "List.turnIntoCSList([" + args.map(this.visit, this).join(", ") + "])";
};
...
Visitor.prototype.fun_if$2 = function (args, modifs) {
    ...
    jscode.push(indent + "if ((" + this.visit(cond) + ").value === true) {");
    this.indent = indent + "    ";
        ...
        case "infix":
            switch (cscode.oper) {
                case ";":
                    ...
                    break;
                case ":=":
                    if (cscode.args[0].ctype !== "function") throw Error("Only function definitions allowed.");
