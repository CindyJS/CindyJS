...
function processCommandLine() {
  var argv = process.argv, n = argv.length, i, arg, key, val;
  for (i = 2; i < n; ++i) {
    arg = argv[i];
    if (/^-/.test(arg)) {
      val = /^-([^=]+)=(.*)$/.exec(arg);
...
function finish() {
  var str = "/** @enum {string} */\n";
  if (settings.varname.indexOf(".") === -1)
    str += "var ";
  str += settings.varname + "={";
  Object.getOwnPropertyNames(data).sort().forEach(function(key) {
    str += "\n" + JSON.stringify(key) + ":" + JSON.stringify(data[key]) + ",";
  });
  str = str.substr(0, str.length - 1) + "\n};";
  if (settings.output)
    fs.writeFileSync(settings.output, str);
  else
    console.log(str);
}
