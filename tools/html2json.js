...
else readStream(fs.createReadStream(input, { encoding: "utf8" }));
...
function transform() {
    ...
    var cindys = [];
    var match;
    var events = ["move", "keydown", "mousedown", "mouseup", "mousedrag", "init", "tick", "draw"];
    while ((match = reScript.exec(content)) !== null) {
        var attr = match[1];
        var script = match[2];
        match = reId.exec(attr);
        if (match) byId[match[1]] = script;
        if (reCindy.test(script)) cindys.push(script);
    }
    if (cindys.length === 1) {
        var fun = Function(
            "CindyJS",
            "createCindy",
            "defaultAppearance", // passed below
            "require",
            "module", // undefined for SOME security
            cindys[0]
        );
        var params = null,
            defaultAppearance = {};
        function dummyCindyJS(arg) {
            params = arg;
        }
        ...
        if (params.defaultAppearance === null) params.defaultAppearance = defaultAppearance;
        var scripts = {},
            scriptpat = params.scripts;
        if (typeof scriptpat !== "string") scriptpat = null;
...
function condense(code) {
    ...
    for (var i = 0; i < code.length; i++) {
        ...
        var c = code[i];
        if (c === '"' && !commentmode) literalmode = !literalmode;

        if (c === "/" && i !== code.length - 1) if (code[i + 1] === "/") commentmode = true;
        if (c === "\n") commentmode = false;
        if (
            !(c === "\u0020" || c === "\u0009" || c === "\u000A" || c === "\u000C" || c === "\u000D" || commentmode) ||
            literalmode
