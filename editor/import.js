...
var Import = {
  ...
  `,
    init: function () {
        document.getElementById("button-import-url").onclick = function () {
            ...
            xmlHttp.onreadystatechange = function () {
                if (xmlHttp.readyState == 4 && xmlHttp.status == 200) Import.fromsource(xmlHttp.responseText);
                else document.getElementById("import-error").innerHTML = "could not load URL";
            };
    ...
    fromsource: function (source) {
        ...
        if (configuration.canvasname) delete configuration.canvasname;

        configuration.exclusive = "true"; // shut down the previous instance

        if (!configuration.use) configuration.use = [];
        ...
        hascolorplot = false;
        for (let s in scripts) {
            if (scripts[s].match(/colorplot/g)) hascolorplot = true;
        }
        if (hascolorplot && configuration.use.indexOf("CindyGL") == -1) configuration.use.push("CindyGL");
