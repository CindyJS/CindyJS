...
var Inspector = {
    ...
    update: function (jsonstr) {
        if (document.getElementById("inspector-window").style.display == "none") return;
        console.log(jsonstr);
        ...
        for (let key in elementswithkey) {
            ...
            if (key == "color") {
                ...
                if (allequal) document.getElementById("colorwheel").classList.remove("different");
                else document.getElementById("colorwheel").classList.add("different");
            } else {
                innerhtml += `<div class=${
                    allequal ? "equal" : "different"
                }><span style="color:rgb(150,150,150)">${elnames}</span> <label for="${key}">${key}: </label>${this.createinput(
                    key,
                    value,
                    elnames,
                    allequal
    ...
    createinput: function (key, value, elements, allequal) {
        ...
        if (typeof value == "boolean") {
            return `<input type="checkbox" name="${key}" ${
                value ? "checked" : ""
            } onchange="Inspector.modifygslp(${elstr}, ${keystr}, this.checked)"></div>`;
            ...
            return `<input type="text" name="${key}" value="[${this.niceprint(
                value
            )}]" onchange="Inspector.modifygslp(${elstr}, ${keystr}, this.value)"></div>`;
        }
