    ...
    evaluator.WebGLvariables = function (args, modifs) {
        ...
        for (var name in modifs) {
            ...
            if (val.ctype == "string") {
                type = val.value;
                if (type == "complex") {
                    text = text + "uniform vec2 " + name + ";\n";
                }
                if (type == "point") {
                    text = text + "uniform vec2 " + name + ";\n";
                }
                if (type == "real") {
                    text = text + "uniform float " + name + ";\n";
                }
                if (type == "homog") {
                    text = text + "uniform vec3 " + name + ";\n";
                }
                if (type == "point3D") {
                    text = text + "uniform vec3 " + name + ";\n";
                }
...
function compile(text) {
    ...
    var vs = createShader(
        "attribute highp vec3 position; void main() { gl_Position = vec4( position, 1.0 ); }",
        gl.VERTEX_SHADER
    );
...
function renderwegbl() {
    ...
    gl.uniform2f(
        gl.getUniformLocation(currentProgram, "translate"),
