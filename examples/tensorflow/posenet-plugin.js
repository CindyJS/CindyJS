CindyJS.registerPlugin(1, "posenet", function (api) {
    ...
    var recreplace = function (ex, rmap) {
        if (ex.ctype === "variable" && rmap[ex.name]) {
            ...
            if (ex.args) ex.args = ex.args.map((e) => recreplace(e, rmap));
            return ex;
        }
    };
    var net = false;
    ...
    function replaceNanByZero(o) {
        ...
        if (typeof o === "object" && o.length !== undefined) return o.map(replaceNanByZero);
        return o;
    }
    ...
    async function getpose(img, cdycallback, px2coord, modifs) {
        ...
        if (!net)
            //net = await posenet.load(handleModif(modifs, 'multiplier', 0.75));
        ...
        //console.log(pose);
        cdypose = wrap(replaceNanByZero(pose.keypoints.map((k) => [k.part, k.score, px2coord(k.position)])));
        api.evaluate(
    ...
    api.defineFunction("estimateSinglePose", 4, function (args, modifs) {
        ...
        var px2coord = function (p) {
            return [
                c.x + (p.x / w) * (b.x - a.x) + (p.y / h) * (b.y - a.y),
                c.y + (p.y / h) * (a.y - c.y) + (p.x / w) * (b.y - a.y),
            ];
        };
