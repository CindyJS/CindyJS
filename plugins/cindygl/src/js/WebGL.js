function usefunction(name) {
  return (args) => {
    //args?
    if (typeof "args" === "string") return `${getPlainName(name)}(${args})`;
    else return `${getPlainName(name)}(${args.join(", ")})`;
  };
}
...
webgl["if"] = (argtypes) => {
  ...
  if (argtypes.length === 2) {
    ...
    if (template) {
      ...
      //they do not have an common lca
      return {
        args: argtypes,
        res: type.voidt,
...
webgl["repeat"] = (argtypes) =>
  (argtypes.length == 2 || argtypes.length == 3) && isconstantint(argtypes[0])
    ? {
        //generator not used yet
        args: argtypes,
...
let glslstructures = [2, 3, 4]
  .map((n) => list(n, type.float))
...
webgl["_"] = (args) => {
  ...
  if (t.type === "list" && isconstantint(args[1])) {
    ...
    if (1 <= Math.abs(k) && Math.abs(k) <= t.length) {
      ...
      //if the certain value is not clear yet.
      return {
        args: args,
        res: t.parameters,
        generator: (x) =>
          console.error(
            `try to access ${k}-th Element of ${t.length}-list ${JSON.stringify(
              args[0]
            )}`
          ),
      };
...
webgl["mult"] = (args) => {
  let match = first([
    ...
    [
      [type.mat2, type.mat2],
      type.mat2,
      useswapinfix("*"), //CindyGL/JS interprets matrixes as lists of rows. Unlike GLSL, where matrices are lists of columns
    ],
  ...
  for (let swap = 0; swap < 2; swap++) {
    ...
    if (
      issubtypeof(args[0 ^ swap], type.float) &&
      (isrvectorspace(args[1 ^ swap]) || iscvectorspace(args[1 ^ swap]))
    ) {
...
webgl["re"] = first([
  [
    [type.complex],
    type.float,
    useincludefunction("realc"), //TODO: shorter x => `(${x}).x`
  ],
]);
...
webgl["randombool"] = first([
  [
    [],
    type.bool,
    (a, modifs, cb) => `(${useincludefunction("random")([], modifs, cb)}>.5)`,
  ],
]);
...
webgl["reverse"] = (args) =>
  args.length === 1 && args[0].type === "list"
    ? {
        args: args,
        res: args[0],
...
webgl["sort"] = (args) =>
  args.length === 1 && depth(args[0]) === 1 && isrvectorspace(args[0])
    ? {
        args: args,
        res: args[0],
