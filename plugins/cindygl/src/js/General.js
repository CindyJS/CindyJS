...
function createPixelArrayFromFloat(pixels) {
  ...
  if (can_use_texture_half_float) return new Uint16Array(pixels.map(toHalf));
  else return new Uint8Array(pixels.map(toByte));
}
...
function createPixelArrayFromUint8(pixels) {
  if (can_use_texture_float) return (new Float32Array(pixels)).map(x => x / 255.);
  if (can_use_texture_half_float) return new Uint16Array((new Float32Array(pixels)).map(x => x / 255.));
