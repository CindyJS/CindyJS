...
CodeBuilder.prototype.computeType = function (expr) {
  ...
  if (expr["isuniform"]) {
    ...
    if (allconstant && expr["impl"]) {
      //use api.evaluateAndVal to compute type of constant expression
      ...
      //if there is something non-constant, we will the functions specified in WebGL.js
...
CodeBuilder.prototype.determineVariables = function (expr, bindings) {
  ...
  function rec(expr, bindings, scope, forceconstant) {
    ...
    for (let i in expr["args"]) {
      let needtobeconstant =
        forceconstant ||
        (expr["oper"] === "repeat$2" && i == 0) ||
        (expr["oper"] === "repeat$3" && i == 0) ||
        (expr["oper"] === "_" && i == 1);
      let nbindings = bindings;
      if (
        ["repeat", "forall", "apply"].indexOf(getPlainName(expr["oper"])) !== -1
      ) {
        if (i == 1) {
          nbindings =
            expr["oper"] === "repeat$2"
          ...
          //take same bindings as for second argument
    ...
    if (expr["oper"] === "=") {
      ...
      let it =
        expr["args"].length === 2
          ? expr["args"][1].bindings["#"]
          : expr["args"][2].bindings[expr["args"][1]["name"]];
      variables[it].assigments.push({
        //add function that accesses first element of array for type detection
        ctype: "infix",
        oper: "_",
        args: [
          expr["args"][0],
...
CodeBuilder.prototype.determineTypes = function () {
  ...
  while (changed) {
    //TODO: implement queue to make this faster
    changed = false;
    for (let v in this.variables)
      if (!this.variables[v].forceconstant) {
...
CodeBuilder.prototype.determineUniforms = function (expr) {
  ...
  function dependsOnPixel(expr) {
    ...
    let alwaysPixelDependent = [
      //Operators that are supposed to be interpreted as pixel dependent;
...
CodeBuilder.prototype.copyRequiredFunctions = function (expr) {
  if (
    expr["ctype"] === "function" &&
    !this.myfunctions.hasOwnProperty(expr["oper"]) &&
    this.api.getMyfunction(expr["oper"]) !== null
  ) {
    //copy and transverse recursively all occuring myfunctions
    let fun = expr["oper"];
...
CodeBuilder.prototype.compile = function (expr, generateTerm) {
  ...
  if (ctype.type === "constant") {
    ...
    if (
      !(
        arraytype.type === "list" ||
        (arraytype.type === "constant" && arraytype.value["ctype"] === "list")
      )
    ) {
    ...
    if (this.myfunctions.hasOwnProperty(fname)) {
      ...
      if (!implementation) {
        console.error(
          `Could not find an implementation for ${fname}(${currenttype
            .map(typeToString)
...
CodeBuilder.prototype.compileFunction = function (fname, nargs) {
  ...
  let code = `${webgltype(this.variables[pname].T)} ${pname}(${vars
    .map(
      (varname) =>
        webgltype(self.variables[bindings[varname]].T) + " " + bindings[varname]
    )
    .join(", ")}){\n`;
