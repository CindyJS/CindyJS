...
let isrvectorspace = (t) =>
    (t.type === "list" && isrvectorspace(t.parameters)) ||
    issubtypeof(t, type.float);
/* checks wheather t can be embedded into an C-vectorspace*/
let iscvectorspace = (t) =>
    (t.type === "list" && iscvectorspace(t.parameters)) ||
...
let finalparameter = (t) =>
    t.parameters !== undefined ? finalparameter(t.parameters) : t;
let dimensionsmatch = (a, b) =>
    depth(a) === depth(b) &&
    (depth(a) === 0 ||
        (a.length === b.length && dimensionsmatch(a.parameters, b.parameters)));
...
let isprimitive = (a) =>
    [type.bool, type.int, type.float, type.complex].indexOf(a) !== -1;

let typesareequal = (a, b) =>
    a === b ||
    (a.type === "constant" &&
        b.type === "constant" &&
        expressionsAreEqual(a.value, b.value)) ||
    (a.type === "list" &&
        b.type === "list" &&
        a.length === b.length &&
        typesareequal(a.parameters, b.parameters));
...
function issubtypeof(a, b) {
    ...
    if (b === type.coordinate2d)
        return (
            issubtypeof(a, type.complex) ||
            issubtypeof(a, type.vec2) ||
            issubtypeof(a, type.point)
        );
    if (b === type.point)
    ...
    if (b === type.color)
        return (
            ...
            (a.type === "list" &&
                ...
                issubtypeof(a.parameters, type.float))
        );
...
function inclusionfunction(toType) {
        ...
        case type.coordinate2d:
            return first([
                ...
                [
                    [type.point],
                    type.coordinate2d,
                    useincludefunction("dehomogenize"),
                ],
            ]);
        default:
            if (toType.type === "list") {
                ...
                return (args) => {
                    let fromType = args[0];
                    let rec = inclusionfunction(toType.parameters)([
                        fromType.parameters,
                    ]).generator;
                    return {
                        args: args,
    ...
    console.log(
        `no inclusionfunction ->${typeToString(
            toType
        )} implemented yet; using identity...`
    );
    return (args) => ({
        args: args,
        res: toType,
...
function webgltype(ctype) {
    ...
    console.error(
        `No WebGL implementation for type ${typeToString(ctype)} found`
    );
}
...
function pastevalue(val, toType) {
    switch (toType) {
        ...
        default:
            console.error(
                `Dont know how to paste values of Type ${typeToString(
                    toType
                )} yet.`
            );
    }
