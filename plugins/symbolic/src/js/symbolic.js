CindyJS.registerPlugin(1, "symbolic", function (api) {
    ...
    var symdiff = function (ex, vmap) {
        if (ex.ctype === "variable") {
            if (vmap[ex.name]) return vmap[ex.name];
            else return real(0);
            ...
            for (let i in ex.args) {
                ...
                if (!isval(dx, 0)) {
                    ...
                    if (!isval(df, 0)) {
                        if (!isval(dx, 1)) df = fun("mult$2", [df, dx]);

                        if (isval(res, 0)) res = df;
                        else res = fun("add$2", [res, df]);
                    }
                }
            }
            return res;
    ...
    var degree = function (ex, vmap) {
        if (ex.ctype === "variable") {
            ...
            if (cdegfun) return cdegfun(ex.args, vmap);
        } else if (ex.ctype === "void") return 0;
    ...
    var csdegree = function (args, modifs) {
        ...
        for (let i in args) {
            if (i >= 1) {
                if (args[i].ctype === "variable") vmap[args[i].name] = 1;
