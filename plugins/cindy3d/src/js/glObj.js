(function (exports) {
    ...
    ShaderProgram.prototype.detectImpl = function (uniform) {
        ...
        if (uniform) n = gl.getProgramParameter(handle, gl.ACTIVE_UNIFORMS);
        else n = gl.getProgramParameter(handle, gl.ACTIVE_ATTRIBUTES);
        for (i = 0; i < n; ++i) {
            ...
            if (info.size > 1) {
                ...
                for (j = 0; j < size; ++j) {
                    name2 = info.name + "[" + j + "]";
                    if (uniform) leaf = this.uniformSetter(name2, info);
                    else leaf = this.attribFactory(name2, info);
                    arr[j] = leaf;
                }
                node[name] = arr;
            } else {
                if (uniform) leaf = this.uniformSetter(info.name, info);
                else leaf = this.attribFactory(info.name, info);
                node[name] = leaf;
            }
        }
        return root;
