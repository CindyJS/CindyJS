...
Controls.prototype.mousemove = function (/** MouseEvent */ evnt) {
    if (evnt.buttons === undefined ? this.mdown : evnt.buttons & 1) {
        if (!isNaN(this.mx)) {
            let dx = evnt.clientX - this.mx,
                dy = evnt.clientY - this.my;
            if (evnt.shiftKey) this.camera.rotateXY(dx, dy);
            else if (evnt.altKey || evnt.ctrlKey || evnt.metaKey) this.camera.translateXY(dx, dy);
            else this.camera.orbitXY(dx, dy);
            this.viewer.render();
        }
        this.mx = evnt.clientX;
        this.my = evnt.clientY;
...
Controls.prototype.wheel = function (/** WheelEvent */ evnt) {
    let d = evnt.deltaY;
    if (d) {
        if (evnt.shiftKey) this.camera.rotateZ(Camera.ROTATE_SENSITIVITY * d);
        else if (evnt.altKey || evnt.ctrlKey || evnt.metaKey) this.camera.translateZ(d);
        else this.camera.zoom(d);
        this.viewer.render();
    }
    evnt.preventDefault();
};
...
Controls.prototype.touchend = function (/** TouchEvent */ evnt) {
    evnt.preventDefault();
    let /** TouchList */ ts = evnt.changedTouches;
    for (let i = 0; i < ts.length; ++i) this.touches.delete(ts[i].identifier);
};
