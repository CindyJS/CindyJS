...
coerce.toReal = function (arg, def = Number.NaN) {
    ...
    let val = arg["value"],
        r = val["real"],
        i = val["imag"];
    if (i !== 0) console.log("complex number is not real");
    return r;
};
...
coerce.toInt = function (arg, def = Number.NaN) {
    ...
    if (i !== 0) console.log("complex number is not real");
    i = Math.round(r);
    if (i !== r) console.log("number is not an integer");
    return i;
};
...
coerce.toString = function (arg, def = null) {
    if (arg["ctype"] === "string") return arg["value"];
...
coerce.toEnum = function (names, arg, def = null) {
    ...
    if (str !== def && names.indexOf(str) !== -1) return str;
...
coerce.toBool = function (arg, def) {
    if (arg["ctype"] === "boolean") return arg["value"];
