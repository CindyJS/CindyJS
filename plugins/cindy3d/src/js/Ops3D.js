...
let Cindy3D = function (api) {
    ...
    function handleModifsAppearance(appearance, modifs, handlers = null) {
        ...
        let key;
        if (handlers) for (key in handlers) combined[key] = handlers[key];
    ...
    defOp("pointshininess3d", 1, function (args, modifs) {
        let shininess = coerce.toInterval(0, 128, evaluate(args[0]));
        if (!isNaN(shininess)) currentInstance.pointAppearance.shininess = shininess;
        return nada;
    });

    defOp("lineshininess3d", 1, function (args, modifs) {
        let shininess = coerce.toInterval(0, 128, evaluate(args[0]));
        if (!isNaN(shininess)) currentInstance.lineAppearance.shininess = shininess;
        return nada;
    });

    defOp("surfaceshininess3d", 1, function (args, modifs) {
        let shininess = coerce.toInterval(0, 128, evaluate(args[0]));
        if (!isNaN(shininess)) currentInstance.surfaceAppearance.shininess = shininess;
        return nada;
    });
    ...
    defOp("draw3d", 1, function (args, modifs) {
        ...
        let appearance = handleModifsAppearance(currentInstance.pointAppearance, modifs);
    ...
    defOp("draw3d", 2, function (args, modifs) {
        ...
        let appearance = handleModifsAppearance(currentInstance.lineAppearance, modifs);
    ...
    defOp("fillpoly3d", 1, function (args, modifs) {
        let lst = coerce.toList(evaluate(args[0]));
        let appearance = handleModifsAppearance(currentInstance.surfaceAppearance, modifs);
        if (lst.length < 2) return nada;
    ...
    function mesh3dImpl(args, modifs) {
        ...
        let appearance = handleModifsAppearance(currentInstance.surfaceAppearance, modifs, {
            normaltype: (a) => (normaltype = coerce.toString(a, normaltype).toLowerCase()),
            colortype: (a) => (colortype = coerce.toString(a, colortype).toLowerCase()),
            topology: (a) => (topology = coerce.toString(a, topology).toLowerCase()),
            colors: (a) => (colors = coerce.toList(a).map((elt) => coerce.toColor(elt))),
            uv: (a) => (uv = coerce.toList(a).map((elt) => coerce.toHomog(elt, [0, 0, 0], 2))),
        ...
        if (args.length === 4) {
            let normals = coerce.toList(evaluate(args[3])).map((elt) => coerce.toDirection(elt));
            ...
            iterMesh(m, n, tcr, tcc, (i1, i2, i3, i4) =>
                currentInstance.triangles.addPolygonAutoNormal([pos[i1], pos[i2], pos[i3], pos[i4]], appearance)
            );
        }
        return nada;
    }
    ...
    defOp("triangle3d", 1, function (args, modifs) {
        ...
        let appearance = handleModifsAppearance(currentInstance.surfaceAppearance, modifs, {
            ...
            normals: (a) => (normals = coerce.toList(a).map((elt) => coerce.toDirection(elt))),
        });
        ...
        if (!colors) colors = [appearance.color, appearance.color, appearance.color];
        if (!normals) {
    ...
    defOp("spotlight3d", 1, function (args, modifs) {
        ...
        currentInstance.lighting.setLight(
            index,
            new SpotLights[frame](position, direction, Math.cos(cutoff), exponent, diffuse, specular)
        );
        return nada;
    });
    ...
    defOp("ambientlight3d", 1, function (args, modifs) {
        ...
        if (color) currentInstance.lighting.ambient = color;
        return nada;
    });
};
