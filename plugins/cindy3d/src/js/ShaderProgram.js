...
ShaderProgram.prototype.detectImpl = function (gl, uniform) {
    ...
    if (uniform) n = /** @type {number} */ (gl.getProgramParameter(handle, gl.ACTIVE_UNIFORMS));
    else n = /** @type {number} */ (gl.getProgramParameter(handle, gl.ACTIVE_ATTRIBUTES));
    for (i = 0; i < n; ++i) {
        if (uniform) info = gl.getActiveUniform(handle, i);
        else info = gl.getActiveAttrib(handle, i);
        ...
        if (info.size > 1) {
            ...
            for (j = 0; j < size; ++j) {
                name2 = info.name + "[" + j + "]";
                if (uniform) leaf = this.uniformSetter(gl, name2, info);
                else leaf = this.attribFactory(gl, name2, info);
                arr[j] = leaf;
            }
            node[name] = arr;
        } else {
            if (uniform) leaf = this.uniformSetter(gl, info.name, info);
            else leaf = this.attribFactory(gl, info.name, info);
            node[name] = leaf;
        }
    }
    return root;
