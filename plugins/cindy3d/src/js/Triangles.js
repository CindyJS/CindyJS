...
Triangles.prototype.addAutoNormal = function (pos1, pos2, pos3, appearance) {
    let n = triangleNormal(pos1, pos2, pos3);
...
Triangles.prototype.addPolygonAutoNormal = function (pos, appearance) {
    if (pos.length == 3) return this.addAutoNormal(pos[0], pos[1], pos[2], appearance);
    let k = pos.length,
        p = Array(k + 2),
        i,
        n = [0, 0, 0];
    for (i = 0; i < k; ++i) p[i] = dehom3(pos[i]);
    p[k] = p[0];
    p[k + 1] = p[1];
    for (i = 1; i <= k; ++i) n = add3(n, cross3(sub3(p[i], p[i - 1]), sub3(p[i], p[i + 1])));
    if (k == 4) {
    ...
    let center = [0, 0, 0, 0],
        prev = pos[k - 1];
    for (i = 0; i < k; ++i) center = add4(center, pos[i]);
    for (i = 0; i < k; ++i) {
...
Triangles.prototype.addPolygonWithNormals = function (pos, n, appearance) {
    if (pos.length == 3) return this.addWithNormals(pos[0], pos[1], pos[2], n[0], n[1], n[2], appearance);
    if (pos.length == 4) {
...
Triangles.prototype.render = function (viewer) {
    ...
    if (this.texture) {
        ...
        if (this.texture.cdyUpdate) this.texture.cdyUpdate();
        ...
        if (
            ...
            this.prevTexture = this.texture;
            this.prevTextureGeneration = this.texture.generation;
            let /** @type {HTMLImageElement|HTMLCanvasElement|HTMLVideoElement} */ img = this.texture.img;
            if (w !== this.texture.width || h !== this.texture.height) {
                if (!this.textureScaler) {
                    this.textureScaler = /** @type {HTMLCanvasElement} */ (document.createElement("canvas"));
