        ...
        let addHemisphereToMesh = function (center, tangent, normal, isStartHemisphere) {
            ...
            for (let lat = 1; lat <= numLatitudeSubdivisions; lat++) {
                ...
                for (let lon = 0; lon < numLongitudeSubdivisions; lon++) {
                    ...
                    let pt = new vec3(Math.cos(theta) * Math.sin(phi), Math.sin(theta) * Math.sin(phi), Math.cos(phi));
            ...
            for (let lat = 0; lat < numLatitudeSubdivisions; lat++) {
                for (let lon = 0; lon < numLongitudeSubdivisions; lon++) {
                    if (isStartHemisphere && lat == 0) {
                        ...
                        indices.push(
                            vertexIndexOffset + (lon % numLongitudeSubdivisions) + lat * numLongitudeSubdivisions
                        );
                        indices.push(
                            vertexIndexOffset + ((lon + 1) % numLongitudeSubdivisions) + lat * numLongitudeSubdivisions
                        );
                        indices.push(
                            vertexIndexOffset + (lon % numLongitudeSubdivisions) + (lat + 1) * numLongitudeSubdivisions
                        );
                        ...
                        indices.push(vertexIndexOffset + 0 + (lat + 1) * numLongitudeSubdivisions);
                    }
                }
            }
        };
