...
TriangleMesh.prototype.saveToObjFile = function (filename) {
    ...
    for (let i = 0; i < this.vertices.length; i++) {
        ...
        fileContent = fileContent + "v " + vertex.x + " " + vertex.y + " " + vertex.z + "\n";
        if (this.normals.length > 0) {
            let normal = this.normals[i];
            fileContent = fileContent + "vn " + normal.x + " " + normal.y + " " + normal.z + "\n";
        }
    }
    ...
    for (let i = 0; i < this.indices.length; i += 3) {
        ...
        fileContent = fileContent + "f " + i1 + "//" + i1 + " " + i2 + "//" + i2 + " " + i3 + "//" + i3 + "\n";
    }
...
TriangleMesh.prototype.saveToAsciiStlFile = function (filename) {
    ...
    for (let i = 0; i < this.indices.length; i += 3) {
        ...
        fileContent = fileContent + "facet normal " + facetNormal.x + " " + facetNormal.y + " " + facetNormal.z + "\n";
        fileContent = fileContent + "\touter loop\n";
        for (let j = 0; j < 3; j++) {
            let vertex = this.vertices[this.indices[i + j]];
            fileContent = fileContent + "\t\tvertex " + vertex.x + " " + vertex.y + " " + vertex.z + "\n";
        }
