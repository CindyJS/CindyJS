...
function runAllTests(files) {
    if (files.length === 0) {
        ...
        fs.readdirSync(refdir).forEach(function (filename) {
            if (filename.match(/\.md$/)) files.push(path.join(refdir, filename));
        });
    }
    files.sort();
...
function runTestFile(filename) {
    ...
    txt.split("\n\n").forEach(function (block) {
        ...
        for (i = 0; i < n; ++i) {
            line = lines[i];
            var mark = line.substr(4, 2),
                rest = line.substr(6);
            if (mark === "> ") {
                if (ininput) {
                    if (curcase.cmd === null) curcase.cmd = rest;
                    else curcase.cmd += "\n" + rest;
                } else {
                    curcase = new TestCase(rest, filename, lineno + i);
                    cases.push(curcase);
                    if (pragmas.length) curcase.pragma = pragmas.slice();
                    ininput = true;
                }
                continue;
            } else if (mark === "G ") {
                ...
                console.log("Unexpected line " + path.basename(filename) + ":" + (lineno + i) + ": " + line);
            }
            ininput = false;
        }
        lineno += n + 1;
    });
    if (!anythingToCheck) return;
...
TestCase.prototype.expectResult = function (str) {
    if (this.expected !== null) console.error("Two results for command " + this.cmd);
    this.expected = str;
};

TestCase.prototype.expectPattern = function (re) {
    if (this.pattern !== null) console.error("Two patterns for command " + this.cmd);
    this.patternSrc = re;
...
function myniceprint(val) {
    if (val.ctype === "string") return JSON.stringify(val.value);
    if (val.ctype === "list") return "[" + val.value.map(myniceprint).join(", ") + "]";
    if (val.ctype === "dict")
        return (
            "{" +
...
function sanityCheck(val) {
    if (typeof val !== "object") throw Error("value must be an object, is " + typeof val);
    if (!val.hasOwnProperty("ctype")) throw Error("value must have a ctype");
    if (typeof val.ctype !== "string") throw Error("ctype is not a string");
    switch (val.ctype) {
        case "string":
            if (typeof val.value !== "string") throw Error("not a string");
            break;
        case "number":
            if (typeof val.value !== "object") throw Error("not a complex number object");
            if (typeof val.value.real !== "number") throw Error("real part is not a number");
            if (typeof val.value.imag !== "number") throw Error("imaginary part is not a number");
            break;
        case "boolean":
            if (typeof val.value !== "boolean") throw Error("not a boolean");
            break;
        case "list":
            if (!Array.isArray(val.value)) throw Error("not an array");
            val.value.forEach(sanityCheck);
            break;
        case "dict":
            if (typeof val.value !== "object") throw Error("not a dict object");
            break;
        case "JSON":
            if (typeof val.value !== "object") throw Error("not a JSON object");
            break;
        case "undefined":
            break;
        case "geo":
            if (typeof val.value !== "object") throw Error("not a geometric element object");
            if (typeof val.value.type !== "string") throw Error("type is not a string");
            if (typeof val.value.kind !== "string") throw Error("kind is not a string");
            break;
        case "error":
            throw val;
        default:
...
TestCase.prototype.run = function () {
    if (this.pragma) {
        for (var p = 0; p < this.pragma.length; ++p) {
            if (/^only /.test(this.pragma[p]) && !/^only [^:]*CindyJS/.test(this.pragma[p])) return true;
            if (/^CindyScript <3\.0/i.test(this.pragma[p])) return true;
        }
    }
    var val,
        actual,
    ...
    try {
        ...
        if (this.exception !== null) {
            actual = e.toString();
            if (this.exception === actual) return true;
...
function FakeCanvas() {
    ...
    Object.keys(this).forEach(function (k) {
        if (k.substr(0, 1) !== "_") this._unchanged[k] = this[k];
    }, this);
}
...
FakeCanvas.prototype.writeLog = function (methodName, args) {
    ...
    this._log.push(methodName + "(" + map.call(args, JSON.stringify).join(", ") + ")");
};
...
module.exports.collectJSON = function (files) {
    ...
    exportJSON = exportJSON.map(function (testcase) {
        return testcase.asJSON();
    });
    return exportJSON;
};
