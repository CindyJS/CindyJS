...
Setting a point _D_ to the center of gravity of three other points.
...
Observe that for setting the coordinate of _D_ the assignment is made to `D.xy`.
The conditional at the end returns color values depending on the position of _D_.

---

##### Selecting and Clustering

Separating points above and below the _x_-axis, and drawing the two clusters.
...
The points above the _x_-axis are distinguished by using the `select` operator with a suitable condition.
The points below the _x_ axes are calculated by using a set difference.
...
Mark the point closest to _A_ by a large green dot.
...
Observe that we have to use the second closest point in the script, since _A_ is closest to itself.
Another solution would be to remove _A_ from the list before sorting it:
...
The function is entered in a Cinderella input field called _Text0_.
...
The lamps are be represented by geometric points _A_, _B_ and _C_.
...
Calculate and draw the Julia set, a well known fractal, determined by the complex number represented by the point _C_.
...
All fish should avoid an obstacle given by a point _U_.
To try this create some masses and make sure that one is labeled _U_ or change the according line in the script.
