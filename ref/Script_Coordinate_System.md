...
**Description:**
In the geometric part of Cinderella you can set a basis to which all drawing is related.
...
By default, all layers are cleared right before the _draw_ part of CindyScript.
However, it is also possible to mark a layer to be not _auto cleared_ which makes it possible to produce background graphics that have to be calculated only once.

#### Setting the drawing layer: `layer(‹int›)`

**Description:**
This statement sets the drawing layer to a specified level.
By default the layer is cleared right before the _draw_ event takes place.
...
**Example:**
You can use non-clearing layers to draw background or foreground graphics once in the _init_ slot instead of drawing them every time in the _draw_ slot of the [construction](The_CindyScript_Editor.md).
...
The code above in the _init_ slot produces a random background once on layer -4.
As the autoclear flag is set to false, this background will stay during all other operations, as demonstrated below.
If you want to achieve the same effect in the _draw_ slot you have to store all random polygons and their colors to recreate them on every cycle.
...
**Description:**
If you work only with a finite part of the projective plane and do not use a spherical view it makes sense to ask for the screen bounds.
...
In most cases, the [`colorplot` function](Function_Plotting.md#colorplot$3) is easier to use and a lot faster.
